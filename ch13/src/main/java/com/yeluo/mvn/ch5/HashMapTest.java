 ...
 * Cloneable, Serializable 1.基于哈希表的 Map 接口的实现。此实现提供所有可选的映射操作，并允许使用 null 值和 null
 * 键。 （除了非同步和允许使用 null 之外，HashMap 类与 Hashtable 大致相同。）此类不保证映射的顺序， 特别是它不保证该顺序恒久不变。
 * 
 * 2.此实现假定哈希函数将元素适当地分布在各桶之间，可为基本操作（get 和 put）提供稳定的性能。 迭代 collection 视图所需的时间与
 * HashMap 实例的“容量”（桶的数量）及其大小（键-值映射关系数）成比例。
 * 所以，如果迭代性能很重要，则不要将初始容量设置得太高（或将加载因子设置得太低）。
 * 
 * 3.HashMap 的实例有两个参数影响其性能：初始容量 和加载因子。容量 是哈希表中桶的数量，初始容量只是哈希表在创建时的容量。 加载因子
 * 是哈希表在其容量自动增加之前可以达到多满的一种尺度。 当哈希表中的条目数超出了加载因子与当前容量的乘积时，则要对该哈希表进行 rehash
 * 操作（即重建内部数据结构）， 从而哈希表将具有大约两倍的桶数。
 * 
 * 4.通常，默认加载因子 (.75) 在时间和空间成本上寻求一种折衷。 加载因子过高虽然减少了空间开销，但同时也增加了查询成本（在大多数 HashMap
 * 类的操作中， 包括 get 和 put 操作，都反映了这一点）。在设置初始容量时应该考虑到映射中所需的条目数及其加载因子， 以便最大限度地减少
 ...
 * 
 * 6.注意，此实现不是同步的。如果多个线程同时访问一个哈希映射，而其中至少一个线程从结构上修改了该映射， 则它必须
 ...
 * 方法来“包装”该映射。 最好在创建时完成这一操作，以防止对映射进行意外的非同步访问，如下所示： Map m =
 * Collections.synchronizedMap(new HashMap(...)); 由所有此类的“collection
 ...
 * 
 * 7.注意，迭代器的快速失败行为不能得到保证，一般来说，存在非同步的并发修改时，不可能作出任何坚决的保证。 快速失败迭代器尽最大努力抛出
 * ConcurrentModificationException。 因此，编写依赖于此异常的程序的做法是错误的，
 * 正确做法是：迭代器的快速失败行为应该仅用于检测程序错误。
 * 
 * 8.此类是 Java Collections Framework 的成员。
 * 
 */
...
implements Map<K, V>, Cloneable, Serializable {
	...
	final class KeySet extends AbstractSet<K> {
		...
		public final Spliterator<K> spliterator() {
			return new KeySpliterator<>(HashMapTest.this, 0, -1, 0, 0);
		}
	...
	final class Values extends AbstractCollection<V> {
		...
		public final Spliterator<V> spliterator() {
			return new ValueSpliterator<>(HashMapTest.this, 0, -1, 0, 0);
		}
	...
	final class EntrySet extends AbstractSet<Map.Entry<K, V>> {
		...
		public final Spliterator<Map.Entry<K, V>> spliterator() {
			return new EntrySpliterator<>(HashMapTest.this, 0, -1, 0, 0);
		}
	...
	@Override
	public Object clone() {
		HashMapTest<K, V> result;
		try {
			result = (HashMapTest<K, V>) super.clone();
	...
	static class HashMapSpliterator<K, V> {
		...
		HashMapSpliterator(HashMapTest<K, V> m, int origin, int fence, int est, int expectedModCount) {
			this.map = m;
			this.index = origin;
			this.fence = fence;
			this.est = est;
	...
	static final class KeySpliterator<K, V> extends HashMapSpliterator<K, V> implements Spliterator<K> {
		KeySpliterator(HashMapTest<K, V> m, int origin, int fence, int est, int expectedModCount) {
	...
	static final class ValueSpliterator<K, V> extends HashMapSpliterator<K, V> implements Spliterator<V> {
		ValueSpliterator(HashMapTest<K, V> m, int origin, int fence, int est, int expectedModCount) {
			super(m, origin, fence, est, expectedModCount);
		}
	...
	static final class EntrySpliterator<K, V> extends HashMapSpliterator<K, V> implements Spliterator<Map.Entry<K, V>> {
		EntrySpliterator(HashMapTest<K, V> m, int origin, int fence, int est, int expectedModCount) {
			super(m, origin, fence, est, expectedModCount);
		}
	...
	static final class TreeNode<K, V> extends LinkedHashMapTest.Entry<K, V> {
		 ...
		 * node.
		 */
		final Node<K, V> untreeify(HashMapTest<K, V> map) {
		 ...
		 */
		final TreeNode<K, V> putTreeVal(HashMapTest<K, V> map, Node<K, V>[] tab, int h, K k, V v) {
			Class<?> kc = null;
		 ...
		 * tree structure).
		 */
		final void removeTreeNode(HashMapTest<K, V> map, Node<K, V>[] tab, boolean movable) {
			int n;
		 ...
		 */
		final void split(HashMapTest<K, V> map, Node<K, V>[] tab, int index, int bit) {
			TreeNode<K, V> b = this;
